...
**antenna** is a command line program you can use as a feed oriented content management system. Historically content management systems were oriented around a unit of content such as a block or page.  With the rise of social media walled gardens the feed (a.k.a. wall, stream) has become an important content unit. Antenna was developed to explore this shift while still supporting historic way the web works. 

### blocks, pages and feeds

In the early web that was either a page of HTML or a block of HTML that was contained in one or more web pages. These concepts remain important. The concept of feed adds the concept of content evolving over time. It is not accidental that the rise og blogs and later podcasts rely on RSS feed. Antenna is a modern static website content management tool that embraces the concept of feed as a pimary organizing concept while still supporting the rest of your static website. While Antenna is oriented to feeds it can be used to implement a blog, wiki or other website organizational structure.
...
The tooling provided by the **antenna** command takes care of kniting together the Markdown documents defining collections and Markdown documents hosting posts. The **antenna** command's intialization process takes care of create the YAML configuration files that further define the organization and appearance of the website leaving you to spend your effort on creating content using the simple and familiar Markdown text notation.
